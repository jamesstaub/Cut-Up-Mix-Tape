...
function Genius(accessToken) {
  ...
  /**
   * Look up referents of a song on Genius.
   * @param  {String}   songId   The song's id. (Does not support created_by_id or web_page_id)
   ...
   *  available.
 **/
  this.getReferents = function(songId, options, callback) {
    if (typeof options === "function") {
      callback = options;
    }
    var request = {
      url: "/referents?song_id=" + songId,
      qs: options
    };
   ...
   */
   ...
   // js hack to include per_page option in search
  this.search = function(query, options, callback) {
    if (typeof options === "function") {
      callback = options;
      var qs = { "q": query }
    }else{
      var qs = { "q": query, per_page: options }
    }
